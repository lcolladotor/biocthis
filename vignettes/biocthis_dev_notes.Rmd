...
bib <- c(
    ...
    usethis = citation("usethis"),
    withr = citation("withr")[1]
)

write.bibtex(bib, file = "biocthis_dev_notes.bib")
```
...
`r Githubpkg("lcolladotor/biocthis")` `r citep(bib[['biocthis']])` was born from my interest to keep using `r CRANpkg("usethis")` and related tools, but in a Bioconductor-friendly way. That is, this is a package that will help me (and maybe others too). This package was born from these 5 issues:
...
Most of the development of the Bioconductor-friendly GitHub Actions workflow provided by `r Githubpkg("lcolladotor/biocthis")` was done with [leekgroup/derfinderPlot/.github/workflows/check-bioc.yml](https://github.com/leekgroup/derfinderPlot/blob/master/.github/workflows/check-bioc.yml) and  [LieberInstitute/recount3/.github/workflows/check-bioc.yml](https://github.com/LieberInstitute/recount3/blob/master/.github/workflows/check-bioc.yml) as detailed at: _Bioconductor-friendly R CMD check action feature suggestion_ [r-lib/actions#84](https://github.com/r-lib/actions/issues/84).

This work eventually lead to `use_bioc_github_action()`. The features of this GHA workflow are described in the `Introduction to biocthis` vignette. Going back to the story about developing this GHA workflow, while working on this GHA workflow, I ran into several issues and I wouldn't be surprised if we run into more of them later on.
...
* I had to learn how to compile `git` from source by modifying [these instructions](https://www.digitalocean.com/community/tutorials/how-to-install-git-on-ubuntu-18-04) in order to have a `git` version equal or newer to 2.18 on Ubuntu 18.04 such that I can then use `actions/checkout@v2` and avoid issues with running `r CRANpkg("pkgdown")`. If you use `actions/checkout@v1` you can end up at [r-lib/actions/issues/50](https://github.com/r-lib/actions/issues/50). If you use the default `git` 2.17.1 on Ubuntu 18.04 then you run into [actions/checkout/issues/238](https://github.com/actions/checkout/issues/238) and other related issues. I could have avoided this by running `r CRANpkg("pkgdown")` on macOS instead of the Bioconductor docker image that is based on [`rockerdev/rstudio:R.4.0.0_ubuntu18.04`](https://github.com/rocker-org/rocker-versioned2/blob/master/dockerfiles/Dockerfile_rstudio_4.0.0-ubuntu18.04).
...
* `r CRANpkg("withr")` `r citep(bib[["withr"]])`
...
```{r createVignette2}
## Clean up
file.remove("biocthis_dev_notes.bib")
```
