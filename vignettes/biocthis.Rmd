...
In this vignette we will create an example package called `biocthispkg` on a temporary directory and work in this directory using `usethis::proj_set()` in order to illustrate how the functions work. In a real world scenario, you would be working inside your R package and thus you wouldn't need to run `usethis::proj_set()`.
...
We highly recommend watching any developments as they happen at `r Githubpkg("r-lib/actions")` since the `r-lib` team does an excellent job keeping their GHA workflows updated. You can do so by subscribing to the [RSS atom feed](https://github.com/r-lib/actions/commits/master.atom) for commits in their repository processed through [RSS FeedBurner](http://feeds.feedburner.com/RecentCommitsToActionsmaster) that you can [subscribe to by email](https://feedburner.google.com/fb/a/mailverify?uri=RecentCommitsToActionsmaster).
